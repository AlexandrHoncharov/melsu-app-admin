...
from models import User, Teacher, VerificationLog, Schedule, ScheduleTeacher, DeviceToken, Room, RetakeSession, RetakeRegistration
...
def get_available_rooms(date_str, time_start, time_end):
    """
    Получение доступных аудиторий на указанное время
    ...
    Returns:
        list: Список доступных аудиторий
    """
    try:
        # Преобразование строковой даты
        date_obj = datetime.datetime.strptime(date_str, '%Y-%m-%d').date()

        # Поиск занятых аудиторий в расписании
        busy_rooms_schedule = db.session.query(Schedule.auditory).filter(
            Schedule.date == date_obj,
            Schedule.time_start < time_end,
            Schedule.time_end > time_start
        ).all()

        # Поиск занятых аудиторий в пересдачах
        busy_rooms_retakes = db.session.query(Room.id).join(
            RetakeSession, Room.id == RetakeSession.room_id
        ).filter(
            RetakeSession.date == date_obj,
            RetakeSession.time_start < time_end,
            RetakeSession.time_end > time_start,
            RetakeSession.status == 'active'
        ).all()

        # Преобразование результатов в списки ID и имен
        busy_rooms_names = [room[0] for room in busy_rooms_schedule if room[0]]
        busy_rooms_ids = [room[0] for room in busy_rooms_retakes]

        # Получение всех активных аудиторий
        all_rooms = Room.query.filter_by(is_active=True).all()

        # Фильтрация доступных аудиторий
        available_rooms = []
        for room in all_rooms:
            # Проверка по ID (для пересдач)
            if room.id in busy_rooms_ids:
                continue

            # Проверка по имени (для регулярного расписания)
            if room.name in busy_rooms_names:
                continue
...
def get_teacher_subjects(teacher_id):
    """
    Получение списка уникальных предметов, которые преподает преподаватель
    ...
    try:
        ...
        # Множество для уникальных предметов
        unique_subjects = set()

        # 1. Прямой поиск по имени преподавателя
        direct_schedules = Schedule.query.filter(
            Schedule.teacher_name.like(f"%{teacher.name}%")
        ).all()
        ...
        # 2. Поиск через таблицу сопоставления ScheduleTeacher
        mapped_schedules = db.session.query(Schedule).join(
            ScheduleTeacher,
            Schedule.teacher_name == ScheduleTeacher.name
        ).filter(
            ScheduleTeacher.mapped_teacher_id == teacher.id
        ).all()
        ...
        print(f"Error getting teacher subjects: {str(e)}")
        return []


def get_student_subjects(student_id):
    """
    Получение списка предметов, на которые записан студент
    ...
    try:
        ...
        # Получаем предметы из расписания для группы студента
        schedules = Schedule.query.filter_by(group_name=student.group).all()

        # Извлекаем уникальные названия предметов
        unique_subjects = set()
        for schedule in schedules:
            if schedule.subject:
                unique_subjects.add(schedule.subject)
        ...
        print(f"Error getting student subjects: {str(e)}")
        return []
...
# =========================================================
# СИСТЕМА ПЕРЕСДАЧ - новые модели данных
# =========================================================
...
# =========================================================
# СИСТЕМА ПЕРЕСДАЧ - вспомогательные функции
# =========================================================
...
# =========================================================
# СИСТЕМА ПЕРЕСДАЧ - API эндпоинты для преподавателей
# =========================================================

@app.route('/api/retakes/teacher/subjects', methods=['GET'])
@token_required
def get_teacher_subjects_api(current_user):
    """
    API для получения списка предметов, которые преподает преподаватель
    """
    if current_user.role != 'teacher':
        return jsonify({'message': 'Доступ запрещен. Требуется роль преподавателя.'}), 403

    try:
        subjects = get_teacher_subjects(current_user.id)
        return jsonify(subjects), 200
    except Exception as e:
        return jsonify({'message': f'Ошибка при получении предметов: {str(e)}'}), 500


@app.route('/api/retakes/rooms/available', methods=['GET'])
@token_required
def get_available_rooms_api(current_user):
    """
    API для получения доступных аудиторий на указанное время
    """
    if current_user.role != 'teacher':
        return jsonify({'message': 'Доступ запрещен. Требуется роль преподавателя.'}), 403
    ...
    try:
        ...
        if not date or not time_start or not time_end:
            return jsonify({'message': 'Не указаны обязательные параметры: date, timeStart, timeEnd'}), 400

        available_rooms = get_available_rooms(date, time_start, time_end)
        ...
        return jsonify({'message': f'Ошибка при получении аудиторий: {str(e)}'}), 500


@app.route('/api/retakes/sessions', methods=['POST'])
@token_required
def create_retake_session(current_user):
    """
    API для создания новой сессии пересдачи
    """
    if current_user.role != 'teacher':
        return jsonify({'message': 'Доступ запрещен. Требуется роль преподавателя.'}), 403
    ...
    try:
        ...
        required_fields = ['subject', 'date', 'timeStart', 'timeEnd', 'roomId']
        for field in required_fields:
            if field not in data:
                return jsonify({'message': f'Не указано обязательное поле: {field}'}), 400

        # Проверка, доступна ли выбранная аудитория
        available_rooms = get_available_rooms(data['date'], data['timeStart'], data['timeEnd'])
        available_room_ids = [room.id for room in available_rooms]

        if int(data['roomId']) not in available_room_ids:
            return jsonify({'message': 'Выбранная аудитория недоступна на указанное время'}), 400

        # Создание новой сессии пересдачи
        new_session = RetakeSession(
        ...
        # Получаем созданную сессию с данными о комнате
        created_session = RetakeSession.query.get(new_session.id)

        # Преобразуем в словарь для ответа
        response_data = {
            'id': created_session.id,
            'teacherId': created_session.teacher_id,
            'teacherName': current_user.full_name or current_user.username,
            'subject': created_session.subject,
            'date': data['date'],
            'timeStart': created_session.time_start,
            'timeEnd': created_session.time_end,
            'roomId': created_session.room_id,
            'roomName': created_session.room.name if created_session.room else None,
            'description': created_session.description,
            'maxStudents': created_session.max_students,
            'status': created_session.status,
            'registrationsCount': 0,
            'createdAt': created_session.created_at.isoformat(),
            'updatedAt': created_session.updated_at.isoformat()
        }

        return jsonify(response_data), 201
    except Exception as e:
        db.session.rollback()
        return jsonify({'message': f'Ошибка при создании пересдачи: {str(e)}'}), 500


@app.route('/api/retakes/teacher/sessions', methods=['GET'])
@token_required
def get_teacher_retake_sessions(current_user):
    """
    API для получения списка сессий пересдач, созданных преподавателем
    """
    if current_user.role != 'teacher':
        return jsonify({'message': 'Доступ запрещен. Требуется роль преподавателя.'}), 403
    ...
    try:
        ...
        # Базовый запрос
        query = RetakeSession.query.filter_by(teacher_id=current_user.id)

        # Применяем фильтр по статусу, если указан
        if status in ['active', 'canceled', 'completed']:
            query = query.filter_by(status=status)

        # Получаем сессии
        sessions = query.order_by(RetakeSession.date, RetakeSession.time_start).all()

        # Преобразуем в список словарей
        sessions_data = []
        for session in sessions:
            # Количество регистраций
            registrations_count = RetakeRegistration.query.filter_by(session_id=session.id).count()
        ...
        return jsonify({'message': f'Ошибка при получении пересдач: {str(e)}'}), 500


@app.route('/api/retakes/sessions/<int:session_id>', methods=['GET'])
@token_required
def get_retake_session_details(current_user, session_id):
    """
    API для получения деталей определенной сессии пересдачи и связанных регистраций
    """
    try:
        # Получаем сессию пересдачи
        session = RetakeSession.query.get_or_404(session_id)

        # Проверяем права доступа
        if current_user.role == 'teacher' and session.teacher_id != current_user.id:
            return jsonify({'message': 'Доступ запрещен. Это не ваша пересдача.'}), 403

        if current_user.role == 'student':
            # Проверяем, есть ли у студента предмет этой пересдачи
            student_subjects = get_student_subjects(current_user.id)

            # Проверяем, что предмет пересдачи есть в списке предметов студента или
            # студент уже записан на эту пересдачу
            student_registered = RetakeRegistration.query.filter_by(
                session_id=session_id,
                student_id=current_user.id
            ).first() is not None

            # Если статус не активный, то проверяем, что студент записан
            if session.status != 'active' and not student_registered:
                return jsonify({'message': 'Доступ запрещен. Пересдача не активна.'}), 403

            # Если статус активный, но это не его предмет и не записан - запрещаем
            if session.status == 'active' and session.subject not in student_subjects and not student_registered:
                return jsonify({'message': 'Доступ запрещен. Это не ваш предмет.'}), 403

        # Преобразуем сессию в словарь
        session_data = {
            'id': session.id,
            'teacherId': session.teacher_id,
            'teacherName': session.teacher.full_name or session.teacher.username if session.teacher else None,
            'subject': session.subject,
            'date': session.date.strftime('%Y-%m-%d'),
            'timeStart': session.time_start,
            'timeEnd': session.time_end,
            ...
            'maxStudents': session.max_students,
            'status': session.status,
            'registrationsCount': len(session.registrations),
            'createdAt': session.created_at.isoformat(),
            'updatedAt': session.updated_at.isoformat()
        }

        # Если пользователь преподаватель, возвращаем информацию о всех регистрациях
        registrations_data = []

        if current_user.role == 'teacher':
            # Получаем все регистрации для сессии
            registrations = RetakeRegistration.query.filter_by(session_id=session_id).all()
            ...
            for registration in registrations:
                registration_data = {
                    ...
                    'studentName': registration.student.full_name or registration.student.username if registration.student else None,
                    'studentGroup': registration.student.group if registration.student else None,
                    'status': registration.status,
                    'notes': registration.notes,
                    'registeredAt': registration.registered_at.isoformat(),
                    'updatedAt': registration.updated_at.isoformat()
                }

                registrations_data.append(registration_data)
        else:
            # Для студента - только его регистрация, если есть
            registration = RetakeRegistration.query.filter_by(
                session_id=session_id,
                student_id=current_user.id
            ).first()
            ...
            if registration:
                registration_data = {
                    ...
                    'sessionId': registration.session_id,
                    'studentId': registration.student_id,
                    'studentName': current_user.full_name or current_user.username,
                    'studentGroup': current_user.group,
                    'status': registration.status,
                    'notes': registration.notes,
                    'registeredAt': registration.registered_at.isoformat(),
                    'updatedAt': registration.updated_at.isoformat()
        ...
        return jsonify({'message': f'Ошибка при получении деталей пересдачи: {str(e)}'}), 500


@app.route('/api/retakes/sessions/<int:session_id>/status', methods=['PUT'])
@token_required
def update_retake_session_status(current_user, session_id):
    """
    API для обновления статуса сессии пересдачи
    """
    if current_user.role != 'teacher':
        return jsonify({'message': 'Доступ запрещен. Требуется роль преподавателя.'}), 403
    ...
    try:
        ...
        if 'status' not in data or data['status'] not in ['active', 'canceled', 'completed']:
            return jsonify({'message': 'Не указан допустимый статус (active, canceled, completed)'}), 400

        # Получаем сессию
        session = RetakeSession.query.filter_by(id=session_id, teacher_id=current_user.id).first()

        if not session:
            return jsonify({'message': 'Пересдача не найдена или у вас нет прав для её изменения'}), 404
        ...
        # При отмене пересдачи, обновляем статус всех ожидающих регистраций
        if data['status'] == 'canceled' and old_status != 'canceled':
            pending_registrations = RetakeRegistration.query.filter_by(
                session_id=session_id,
                status='pending'
            ).all()
            ...
            for registration in pending_registrations:
                ...
                registration.notes = f"{registration.notes or ''}\n\nПересдача была отменена преподавателем.".strip()
                registration.updated_at = datetime.datetime.utcnow()

            db.session.commit()

        # Преобразуем обновленную сессию в словарь
        session_data = {
            'id': session.id,
            'teacherId': session.teacher_id,
            'teacherName': current_user.full_name or current_user.username,
            ...
            'maxStudents': session.max_students,
            'status': session.status,
            'registrationsCount': RetakeRegistration.query.filter_by(session_id=session_id).count(),
            'createdAt': session.created_at.isoformat(),
            'updatedAt': session.updated_at.isoformat()
        }
        ...
        db.session.rollback()
        return jsonify({'message': f'Ошибка при обновлении статуса пересдачи: {str(e)}'}), 500


@app.route('/api/retakes/registrations/<int:registration_id>/status', methods=['PUT'])
@token_required
def update_registration_status(current_user, registration_id):
    """
    API для обновления статуса регистрации на пересдачу (одобрение или отклонение)
    """
    if current_user.role != 'teacher':
        return jsonify({'message': 'Доступ запрещен. Требуется роль преподавателя.'}), 403
    ...
    try:
        ...
        if 'status' not in data or data['status'] not in ['approved', 'rejected']:
            return jsonify({'message': 'Не указан допустимый статус (approved, rejected)'}), 400

        # Получаем регистрацию с проверкой, что сессия принадлежит текущему преподавателю
        registration = RetakeRegistration.query.join(
            RetakeSession, RetakeRegistration.session_id == RetakeSession.id
        ).filter(
            RetakeRegistration.id == registration_id,
            RetakeSession.teacher_id == current_user.id
        ).first()

        if not registration:
            return jsonify({'message': 'Регистрация не найдена или у вас нет прав для её изменения'}), 404

        # Обновляем статус
        registration.status = data['status']

        # Добавляем примечание, если указано
        if 'notes' in data and data['notes']:
            teacher_note = f"Примечание преподавателя: {data['notes']}"
            if registration.notes:
                registration.notes = f"{registration.notes}\n\n{teacher_note}"
            else:
                registration.notes = teacher_note

        registration.updated_at = datetime.datetime.utcnow()

        db.session.commit()

        # Преобразуем обновленную регистрацию в словарь
        registration_data = {
            'id': registration.id,
            'sessionId': registration.session_id,
            'studentId': registration.student_id,
        ...
        db.session.rollback()
        return jsonify({'message': f'Ошибка при обновлении статуса регистрации: {str(e)}'}), 500


# =========================================================
# СИСТЕМА ПЕРЕСДАЧ - API эндпоинты для студентов
# =========================================================

@app.route('/api/retakes/student/available-sessions', methods=['GET'])
@token_required
def get_student_available_retake_sessions(current_user):
    """
    API для получения доступных для студента сессий пересдач
    """
    if current_user.role != 'student':
        return jsonify({'message': 'Доступ запрещен. Требуется роль студента.'}), 403
    ...
    try:
        ...
        # Получаем все активные пересдачи для предметов студента, которые еще не прошли
        sessions = RetakeSession.query.filter(
            RetakeSession.subject.in_(student_subjects),
            RetakeSession.status == 'active',
            db.or_(
                RetakeSession.date > today,
                db.and_(
                    RetakeSession.date == today,
                    # Если дата сегодня, проверяем, что время окончания еще не наступило
                    RetakeSession.time_end > datetime.datetime.now().strftime('%H:%M')
                )
            )
        ).order_by(RetakeSession.date, RetakeSession.time_start).all()

        # Получаем ID сессий, на которые студент уже записан
        registered_session_ids = db.session.query(RetakeRegistration.session_id).filter_by(
            student_id=current_user.id
        ).all()
        registered_session_ids = [id[0] for id in registered_session_ids]

        # Фильтруем сессии, исключая те, на которые студент уже записан
        available_sessions = [session for session in sessions if session.id not in registered_session_ids]
        ...
        return jsonify({'message': f'Ошибка при получении доступных пересдач: {str(e)}'}), 500


@app.route('/api/retakes/student/register', methods=['POST'])
@token_required
def register_for_retake_session(current_user):
    """
    API для регистрации студента на пересдачу
    """
    if current_user.role != 'student':
        return jsonify({'message': 'Доступ запрещен. Требуется роль студента.'}), 403
    ...
    try:
        ...
        if 'sessionId' not in data:
            return jsonify({'message': 'Не указан ID сессии пересдачи'}), 400

        session_id = data['sessionId']

        # Проверяем, существует ли сессия и активна ли она
        session = RetakeSession.query.filter_by(id=session_id, status='active').first()

        if not session:
            return jsonify({'message': 'Пересдача не найдена или не активна'}), 404

        # Проверяем, не прошла ли уже пересдача
        if session.date < datetime.date.today():
            return jsonify({'message': 'Невозможно записаться на прошедшую пересдачу'}), 400

        if session.date == datetime.date.today() and session.time_end < datetime.datetime.now().strftime('%H:%M'):
            return jsonify({'message': 'Невозможно записаться на прошедшую пересдачу'}), 400

        # Проверяем, что предмет в списке предметов студента
        student_subjects = get_student_subjects(current_user.id)

        if session.subject not in student_subjects:
            return jsonify(
                {'message': 'Невозможно записаться на пересдачу по предмету, которого нет в вашем расписании'}), 400

        # Проверяем, не записан ли уже студент на эту пересдачу
        existing_registration = RetakeRegistration.query.filter_by(
            session_id=session_id,
            student_id=current_user.id
        ).first()

        if existing_registration:
            return jsonify({'message': 'Вы уже записаны на эту пересдачу'}), 400

        # Проверяем, не достигнуто ли максимальное количество студентов
        if session.max_students:
            current_registrations = RetakeRegistration.query.filter_by(session_id=session_id).count()

            if current_registrations >= session.max_students:
                return jsonify({'message': 'Достигнуто максимальное количество студентов на этой пересдаче'}), 400

        # Создаем новую регистрацию
        new_registration = RetakeRegistration(
            session_id=session_id,
            student_id=current_user.id,
            status='pending',
        ...
        # Преобразуем созданную регистрацию в словарь
        registration_data = {
            'id': new_registration.id,
            'sessionId': new_registration.session_id,
            'studentId': new_registration.student_id,
            'studentName': current_user.full_name or current_user.username,
            'studentGroup': current_user.group,
            'status': new_registration.status,
            'notes': new_registration.notes,
            'registeredAt': new_registration.registered_at.isoformat(),
            'updatedAt': new_registration.updated_at.isoformat()
        }

        return jsonify(registration_data), 201
    except Exception as e:
        db.session.rollback()
        return jsonify({'message': f'Ошибка при регистрации на пересдачу: {str(e)}'}), 500


@app.route('/api/retakes/student/registrations', methods=['GET'])
@token_required
def get_student_registrations(current_user):
    """
    API для получения регистраций студента на пересдачи
    """
    if current_user.role != 'student':
        return jsonify({'message': 'Доступ запрещен. Требуется роль студента.'}), 403

    try:
        # Получаем все регистрации студента с сессиями
        registrations = db.session.query(RetakeRegistration, RetakeSession).join(
            RetakeSession, RetakeRegistration.session_id == RetakeSession.id
        ).filter(
            RetakeRegistration.student_id == current_user.id
        ).order_by(
            RetakeSession.date.desc(),
            RetakeSession.time_start
        ).all()

        # Преобразуем в список словарей
        registrations_data = []
        for registration, session in registrations:
            registration_data = {
                'registration': {
                    'id': registration.id,
                    'sessionId': registration.session_id,
        ...
        return jsonify({'message': f'Ошибка при получении регистраций: {str(e)}'}), 500


@app.route('/api/retakes/student/registrations/<int:registration_id>', methods=['DELETE'])
@token_required
def cancel_registration(current_user, registration_id):
    """
    API для отмены регистрации на пересдачу
    """
    if current_user.role != 'student':
        return jsonify({'message': 'Доступ запрещен. Требуется роль студента.'}), 403
    ...
    try:
        ...
        if not registration:
            return jsonify({'message': 'Регистрация не найдена или у вас нет прав для её отмены'}), 404

        # Проверяем, что статус регистрации "pending" (в ожидании)
        if registration.status != 'pending':
            return jsonify({'message': 'Можно отменить только регистрации со статусом "в ожидании"'}), 400

        # Проверяем, не прошла ли уже пересдача
        session = RetakeSession.query.get(registration.session_id)

        if not session:
            return jsonify({'message': 'Сессия пересдачи не найдена'}), 404

        if session.date < datetime.date.today():
            return jsonify({'message': 'Невозможно отменить регистрацию на прошедшую пересдачу'}), 400
        ...
        db.session.rollback()
        return jsonify({'message': f'Ошибка при отмене регистрации: {str(e)}'}), 500

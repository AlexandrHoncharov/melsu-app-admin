...
class RetakeSession(db.Model):
    """Модель для сессии пересдачи"""
    __tablename__ = 'retake_session'

    id = db.Column(db.Integer, primary_key=True)
    teacher_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    subject = db.Column(db.String(255), nullable=False)
    date = db.Column(db.Date, nullable=False)
    time_start = db.Column(db.String(5), nullable=False)  # Format: HH:MM
    time_end = db.Column(db.String(5), nullable=False)  # Format: HH:MM
    room_id = db.Column(db.Integer, db.ForeignKey('room.id'), nullable=False)
    description = db.Column(db.Text, nullable=True)
    max_students = db.Column(db.Integer, nullable=True)
    status = db.Column(db.String(20), default='active')  # active, canceled, completed
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    # Отношения
    teacher = db.relationship('User', backref='retake_sessions')
    registrations = db.relationship('RetakeRegistration', backref='session', cascade='all, delete-orphan')


class RetakeRegistration(db.Model):
    """Модель для записи на пересдачу"""
    __tablename__ = 'retake_registration'

    id = db.Column(db.Integer, primary_key=True)
    session_id = db.Column(db.Integer, db.ForeignKey('retake_session.id'), nullable=False)
    student_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    ...
    # Отношения
    student = db.relationship('User', backref='retake_registrations')

    # Уникальность: студент может записаться на каждую пересдачу только один раз
    __table_args__ = (
